...
class FCCLIPHead(nn.Module):
    ...
    def get_image_embeddings(self, features, mask=None):
        return self._get_mask_embeddings(features, mask)
    ...
    def _get_mask_embeddings(self, features, mask=None):
        mask_features, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(features)
        if self.transformer_in_feature == "multi_scale_pixel_decoder":
            # self.predictor is MultiScaleMaskedTransformerDecoder in fcclip_transformer_decoder.py
            predictions = self.predictor.get_mask_embeddings(multi_scale_features, mask_features, mask, text_classifier=features["text_classifier"])
        else:
            raise NotImplementedError
        return predictions
