...
def init_clip(clip_version="ViT-B/32"):
    ...
    return clip_model, clip_feat_dim
...
def get_lseg_score(
    ...
    if use_multiple_templates:
        ...
        # average the features
        if avg_mode == 0:
            text_feats = text_feats.reshape((-1, len(mul_tmp), text_feats.shape[-1]))   #split in two: query / other
            text_feats = np.mean(text_feats, axis=1)
